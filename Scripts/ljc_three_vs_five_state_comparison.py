...
cav_freq_array = np.linspace(1.75,2.25,1001)
spectrum = np.copy(cav_freq_array)
...
[a_0,a_1,a_2,a_3],polarizance = ljc.brown_params_from_raw(dielectric_params,dipole_matrix,energy_array,spectrum,length_by_energy)
chiral_factor = ljc.chiral_factor(a_1,length_by_energy,cav_freq_array,energy_array,
                                  dip_mags,dip_angles,w_m_at_cavity_freq=w_m,xi = prefactor,style = "simplified")
...
e_x_three_state, a_x_three_state= hamiltonian_three_state_sweep(cav_freq_array,chiral_factor,acd_coupling,trans_freq)

e_x_five_state,a_x_five_state= hamiltonian_five_state_sweep(cav_freq_array,chiral_factor,valley_coupling,acd_coupling,trans_freq,valley_freq)
...
midpoint = int(np.size(cav_freq_array)/2)+1
print(np.abs(helicity_five_state_masked[1,midpoint]))